...
if (!empty($unameOfUser) && !empty($passOfUser))
{
    ...
    $userQuery=$connection->CheckUser($conobj,"roomser",$unameOfUser,$passOfUser);
...
if(isset($_POST['update']))
{
    ...
    $connection = new db();
    $conobj=$connection->OpenCon();
    $userQuery=$connection->UpdateUser($conobj,"roomser",$nameOfUser,$unameOfUser,$emailOfUser,$contactOfUser,$genderOfUser,$passOfUser);
    if ($userQuery==TRUE)
    {
...
if(isset($_POST['delete']))
{
    if($unameOfUser != "")
    {
        $connection = new db();
        $conobj=$connection->OpenCon();
        $userQuery=$connection->DeleteUser($conobj,"roomser",$unameOfUser);
        $connection->CloseCon($conobj);
